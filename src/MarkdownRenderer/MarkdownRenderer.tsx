...
export interface MarkdownRendererProps {
  ...
  /** Called when user clicks "Convert to Mermaid" on a code block */
  onConvertToMermaid?: (code: string) => void;
}
...
export const MarkdownRenderer = ({
  ...
  return (
    <div className={wrapperClass} onClick={handleClick}>
      {blocks.map((block, i) => (
        <BlockRenderer key={i} block={block} onLinkClick={onLinkClick} onConvertToMermaid={onConvertToMermaid} />
      ))}
    </div>
  );
};
...
function BlockRenderer({ block, onLinkClick, onConvertToMermaid }: { block: Block; onLinkClick?: (href: string) => void; onConvertToMermaid?: (code: string) => void }) {
  switch (block.type) {
    case 'heading':
    ...
    case 'code':
      return <CodeBlock code={block.code} language={block.language || undefined} copyable exportable exportImage onConvertToMermaid={onConvertToMermaid} />;
    case 'frontmatter':
      return (
        <div className="markdown-renderer__frontmatter">
          <div className="markdown-renderer__frontmatter-header">Metadata</div>
          <table className="markdown-renderer__frontmatter-table">
            <tbody>
              {Object.entries(block.data).map(([key, value]) => (
                <tr key={key}>
                  <td className="markdown-renderer__frontmatter-key">{key}</td>
                  <td className="markdown-renderer__frontmatter-value">{String(value)}</td>
                </tr>
              ))}
            </tbody>
          </table>
